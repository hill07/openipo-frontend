...
export async function getServerSideProps() {
  try {
    ...
    // Filter for Closed IPOs using status derivation
    const closedIPOs = ipos.filter(ipo => {
    ...
    closedIPOs.sort((a, b) => {
      const aClose = a.dates?.close ? new Date(a.dates.close) : new Date(0);
      const bClose = b.dates?.close ? new Date(b.dates.close) : new Date(0);
      return bClose - aClose;
    });
    
    return { props: { ipos: closedIPOs } };
  } catch (err) {
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="List of recently closed IPOs in India."
        />
        <meta property="og:title" content="Closed IPOs | OpenIPO" />
        <meta property="og:description" content="Track recently closed IPOs" />
      </Head>

      <div className="page-container">
        <div className="page-header">
          <h1 className="page-title">Closed IPOs ({filteredIPOs.length})</h1>
